...
class Digital:
    ...
    def save(self, cursor):
        try:
            sql = """
            INSERT INTO digital (id, date_const, personnel_id, type_libelle, nom_client, prenom_client, phone_client, email_client, items, quantite, prix_unit, montant_ht, tva, montant_ttc, modalite_paiement, type_paiement, observations) VALUES 
    ...
    def update(self, cursor):
        try:
            sql = """UPDATE digital SET date_const=%s, personnel_id=%s, type_libelle=%s, nom_client=%s, prenom_client=%s, phone_client=%s, email_client=%s, items=%s, quantite=%s, prix_unit=%s, montant_ht=%s, tva=%s, montant_ttc=%s, modalite_paiement=%s, type_paiement=%s, observations=%s WHERE id = %s
            """
